...
namespace NINA.Equipment.Equipment.MyRotator {
    ...
    internal class IndigoRotator : IndigoDevice<IINDIRotator>, IRotator, IDisposable {

        public IndigoRotator(INDIDeviceInfo info, IProfileService profileService) : base(info) {
        ...
        public void Sync(float skyAngle) {
            ...
            Synced = true;
            Logger.Debug($"INDIGO - Mechanical Position is {MechanicalPosition}° - Sync Position to Sky Angle {skyAngle}° using offset {offset}");
        }

        public async Task<bool> Move(float angle, CancellationToken ct) {
            if (ShouldBeConnected) {
                if (angle >= 360) angle = AstroUtil.EuclidianModulus(angle, 360);
                if (angle <= -360) angle = AstroUtil.EuclidianModulus(angle, -360);
                Logger.Debug($"INDIGO - Move relative by {angle}° - Mechanical Position reported by rotator {MechanicalPosition}° and offset {offset}");
        ...
        protected override IINDIRotator GetInstance() {
            return device ??= new NINA.Indigo.Devices.IndigoRotator(_device);
        }
    }
}
