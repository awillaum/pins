...
namespace NINA.Equipment.Equipment {
    ...
    /// <summary>
    /// Base equipment adapter for INDIGO devices.
    /// Mirrors <see cref="IndiDevice{DeviceT}"/> but registers devices against
    /// <see cref="IndigoClient"/> and labels them with the "INDIGO" category.
    /// </summary>
    public abstract class IndigoDevice<DeviceT> : BaseINPC, IDevice where DeviceT : IINDIDevice {

        private string indigoRegistrationName;
        protected readonly INDIDeviceInfo _device;

        public IndigoDevice(INDIDeviceInfo info) {
            this.Category = "INDIGO";
            _device = info;
            Id = info.Id;
            indigoRegistrationName = info.Name;
        ...
        private string name;
        public string Name => name ?? indigoRegistrationName;

        public string DisplayName { get; private set; }

        public string Description {
            get {
                try { return device?.Description ?? "INDIGO Device"; } catch { }
                return string.Empty;
            }
        }
        ...
        public string DriverInfo {
            get {
                try { return device?.DriverInfo ?? "INDIGO Driver"; } catch { }
                return string.Empty;
            }
        }
        ...
        public string DriverVersion {
            get {
                try { return device?.DriverVersion ?? "1.0"; } catch { }
                return string.Empty;
            }
        }
        ...
        private void DisconnectOnConnectionError() {
            try { Disconnect(); } catch (Exception ex) { Logger.Error(ex); }
        }

        private bool TryReconnect() {
            var indigoConnectionState = device.GetSwitchPropertyValue("CONNECTION", "CONNECT");
            if (indigoConnectionState != true) {
                Logger.Warning($"{Name} detected as externally disconnected (INDIGO CONNECTION/CONNECT ≠ true) — treating as real disconnect");
            ...
            return Task.Run(async () => {
                try {
                    ...
                    if (Connected) {
                        ...
                        if (name == null) {
                            try {
                                name = string.IsNullOrEmpty(device?.Name) ? indigoRegistrationName : device?.Name;
                                DisplayName = $"{name} (INDIGO)";
                            } catch {
                                name = indigoRegistrationName;
                            }
                        }
        ...
        public void Disconnect() {
            try {
                ...
                PreDisconnect();
                try { Connected = false; } catch (Exception ex) { Logger.Error(ex); }
                lock (lockObj) {
                    connectedExpectation = false;
                    InvalidatePropertyCache();
                }
                PostDisconnect();
                name = null;
                DisplayName = $"{indigoRegistrationName} ({Category})";
            } finally {
                try { Dispose(); } catch { }
            }
        }
